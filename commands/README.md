...
Commands are user-facing entry points that map to skills. They do not duplicate skill instructions.

Type `/<name>` to invoke one. Claude Code also selects a skill on its own when the request matches its description, so the slash form is for forcing a specific method.
