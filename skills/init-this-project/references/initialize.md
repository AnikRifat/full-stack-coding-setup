...
Source: Anik requested that he can invoke `loop-zero-to-production` in a blank folder to initialize the project. His default for a new business portfolio is Laravel + Livewire. Read the coordinator's [product method](../../loop-zero-to-production/references/product-method.md), [arch-design](../../arch-design/SKILL.md), and the applicable capability guidance as the task requires.
