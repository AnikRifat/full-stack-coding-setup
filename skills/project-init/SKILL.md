...
Use this skill to turn a blank or instruction-only folder into a locally verified software project. It owns native project setup, not the global Codex or Claude environment. Read the user profile, [project initialization](references/initialize.md), [output rules](../../rules/output.md), and the coordinator's [deploy-ready handover method](../loop-delivery/references/delivery-handover.md) before choosing a setup path.
