...
Inspect the repository, project context, requirements, and existing conventions before proposing a boundary or pattern. Read [architecture guidance](references/architecture.md), [selection](references/selection.md) when choosing a starting shape for a new project or a material restructuring, [core rules](../../rules/coding.md), [output rules](../../rules/output.md), and only the capability guidance that matches the actual stack. For a schema, migration, query, or data-modeling decision read [databases](../../capabilities/databases.md). For a boundary that carries authentication, money, personal data, or tenancy read [security](../../capabilities/security.md).
