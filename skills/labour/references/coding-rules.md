...
Before committing to a material design, compare it with these rules. Before finishing an increment, run the checks affected by the change. Repair supported failures and continue through [operating-loop.md](../../loop-zero-to-production/references/operating-loop.md); broader checks are warranted by changed behavior, failures, or unresolved concerns rather than repeated ceremony.
