...
Read [Labour simplicity](simplicity.md) before architecture or dependency decisions. Use the coordinator's [product method](../../loop-zero-to-production/references/product-method.md), [domains.md](../../loop-zero-to-production/references/domains.md), and [coding-rules.md](coding-rules.md) to connect software structure to the actual journey, actors, business rules, and engineering requirements.
...
Feed the selected architecture into [plan-do](../../plan-do/SKILL.md) and Labour's [build mode](build.md), then validate it through implementation evidence. Revise it when actual results expose a limitation, rather than prebuilding every possible extension.
