...
Labour incorporates Anik's adaptation of [Ponytail](https://github.com/DietrichGebert/ponytail), by Dietrich Gebert. The upstream source checkout is retained as `lazyagent/` in this library workspace. Preserve the upstream MIT notice in [LICENSE](../LICENSE).
...
If a deliberately simpler implementation has a real operational ceiling, record that ceiling and what evidence would justify an upgrade. A concise `labour:` comment is useful when the limitation belongs beside the code; otherwise use existing decision notes. Ordinary straightforward code needs no such marker.
