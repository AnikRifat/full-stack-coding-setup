...
Use this coordinator's [product method](product-method.md) for an actual missing product decision, [plan-do](../../plan-do/SKILL.md) for goal-driven delivery planning, [init-this-project](../../init-this-project/SKILL.md) for a new project setup, and [Labour](../../labour/SKILL.md) for architecture, implementation, debugging, or review. Use [init-this-pc](../../init-this-pc/SKILL.md) only when global Codex/Claude scratch-environment work is explicitly requested or a verified environment blocker needs bounded attention. Labour applies its simplicity approach and relevant coding rules. Read the selected entry point before its resources.
