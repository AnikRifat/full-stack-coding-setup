...
| An existing React/Next.js project receives a coding request. | Preserves its stack and installed version, loads applicable official guidance through `labour`, and does not switch it to Laravel |
| The main workflow reaches an agreed implementation increment. | Reads `labour/SKILL.md`, its strict rules, Labour, and applicable framework resources; coding has its own entry point and deliverable |
...
| “Use `$labour` to implement this agreed screen.” The brief, architecture, and app already exist. | Completes the scoped verified implementation; does not begin unrelated discovery, initialization, or full lifecycle delivery |
...
| “Use `$labour` to assess the integration boundary, implement a scoped fix, and review the result.” | Uses one engineering skill with the applicable architecture, build, debugging, or review reference; applies simplicity, strict rules, and Karpathy-inspired guidance without treating the old skills as installed dependencies |
| A legacy request names the former engineering micro-skills. | Routes it to Labour, preserves the requested engineering outcome, and does not claim a removed skill was loaded |
