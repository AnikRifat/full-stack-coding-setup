...
- Work from a blank folder to initialize the project, then own requested lifecycle work, including architecture and coding. `machine-init` covers a requested global Codex/Claude scratch environment; `project-init` covers the application itself.
...
- On 2026-09-17, Anik explicitly separated global Codex/Claude scratch setup into `machine-init` and application setup into `project-init`. The PC skill inspects actual local tools and configuration rather than assuming a Claude setup.
